...
import { useDelFeatureMutation, useSaveFeatureMutation, useShowAllAdminFeatureQuery, useUpdateFeaturesMutation } from "../../app/Api/Feature";
...
const FeatureAdmin = () => {
  ...
  const [saveServices , {isLoading:loadingSave}] = useSaveFeatureMutation();
  const [updateServices, {isLoading:loading}] = useUpdateFeaturesMutation();
  const [delServices, {isLoading:loadingDel}] = useDelFeatureMutation();
  const { data: services, error, isLoading ,refetch} = useShowAllAdminFeatureQuery(currentPage); 
  ...
  const columns = [
    { title: "title (Arabic)", dataIndex: "tittle_ar", key: "tittle_ar", render: (text) => text ,width:150},
    { title: "title (English)", dataIndex: "tittle_en", key: "tittle_en", render: (text) => text ,width:150},
    { title: "Description (Arabic)", dataIndex: "desc_ar", key: "desc_ar", render: (text) => text ,width:250},
    { title: "Description (English)", dataIndex: "desc_en", key: "desc_en", render: (text) => text ,width:250},
    {
      title: "Image",
      dataIndex: "image",
      key: "image",
  ...
  return (
    <Box sx={{ height: 500, width: "100%" }}>
      ...
      <Box sx={{ height: "auto", width: "100%" }} className="cta">
        <Table
          dataSource={services?.feature?.data}
          columns={columns}
          rowKey="id"
          loading={isLoading}
          error={error}
          ...
          pagination={{
            total: services?.feature?.total || 0,
            current: currentPage,
            pageSize: pageSize,
            onChange: (page) => {
              setCurrentPage(page);
      ...
      <StyledModal open={isModalVisible} onClose={handleCloseModal}>
        <StyledBox>
          <Typography variant="h6" gutterBottom>
            {editingCategory ? "Edit Feature" : "Add Feature"}
          </Typography>
