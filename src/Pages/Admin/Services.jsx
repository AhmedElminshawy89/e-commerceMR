...
const Services = () => {
  ...
  const [saveServices , {isLoading:loadingSave}] = useSaveServicesMutation();
  const [updateServices, {isLoading:loading}] = useUpdateServicesMutation();
  const [delServices, {isLoading:loadingDel}] = useDelServicesMutation();
  const { data: services, error, isLoading ,refetch} = useShowAllAdminServicesQuery(currentPage); 
    ...
    <Box sx={{ height: 500, width: "100%" }}>
      ...
      <Box sx={{ height: "auto", width: "100%" }} className="cta">
        <Table
          ...
          pagination={{
            total: services?.services?.total || 0,
            current: currentPage,
            pageSize: pageSize,
            onChange: (page) => {
              setCurrentPage(page);
      ...
      <StyledModal open={isModalVisible} onClose={handleCloseModal}>
        <StyledBox>
          <Typography variant="h6" gutterBottom>
            {editingCategory ? "Edit Services" : "Add Services"}
          </Typography>
