...
import { useDelAdvertiseMutation, useSaveAdvertiseMutation, useShowAllAdminAdvertiseQuery, useUpdateAdvertiseMutation, useUpdateStatusAdvertiseMutation } from "../../app/Api/Advertising";
...
const Advertising = () => {
  ...
  const [saveServices , {isLoading:loadingSave}] = useSaveAdvertiseMutation();
  const [updateServices, {isLoading:loading}] = useUpdateAdvertiseMutation();
  const [updateStatusServices, {isLoading:loadingStatus}] = useUpdateStatusAdvertiseMutation();
  const [delServices, {isLoading:loadingDel}] = useDelAdvertiseMutation();
  const { data: advertise, error, isLoading ,refetch} = useShowAllAdminAdvertiseQuery(currentPage); 
  ...
  return (
    <Box sx={{ height: 500, width: "100%" }}>
      ...
      <Box sx={{ height: "auto", width: "100%" }} className="cta">
        <Table
          dataSource={advertise?.advertise?.data}
          columns={columns}
          rowKey="id"
          loading={isLoading}
          error={error}
          ...
          pagination={{
            total: advertise?.advertise?.total || 0,
            current: currentPage,
            pageSize: pageSize,
            onChange: (page) => {
              setCurrentPage(page);
      ...
      <StyledModal open={isModalVisible} onClose={handleCloseModal}>
        <StyledBox>
          <Typography variant="h6" gutterBottom>
            {editingCategory ? "Edit Advertising" : "Add Advertising"}
          </Typography>
  ...
  <Box sx={{ marginBottom: 2 }}>
    ...
    />
      ...
      <Dialog open={openDeleteDialog} onClose={cancelDeleteCategory}>
        <DialogTitle>Delete Advertising</DialogTitle>
        <DialogContent>
          <Typography>Are you sure you want to delete this Advertising?</Typography>
        </DialogContent>
        <DialogActions>
